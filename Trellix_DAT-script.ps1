#Trellix DAT latest patching script
...
if (Test-Path $installerPath) {
    ...
    #step 2: Install Trellix DAT silently
    Write-Host "Installing Trellix DAT silently..."
    Start-Process -FilePath $installerPath -ArgumentList "/sAll /rs /msi /norestart" -wait

    #step 3: Verify if installation was successful
    $TrellixDatInstalled = Get-WmiObject -Class win32_Product | Where-Object { $_.Name -like "Trellix DAT"}

    if ($McafeePlatformInstalled) {
    Write-Host "Trellix DAT installed successfully." 
    }else {
    Write-Host "Trellix DAT installation failed." 
    }
